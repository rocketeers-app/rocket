...
class ImportRemoteDatabase
{
    ...
    public function handle($site, $server = null)
    {
        ...
        $process = Process::fromShellCommandline('ssh rocketeer@'.$server.' "sudo mysqldump --host=\''.$host.'\' --user=\''.$username.'\' --password=\''.$password.'\' --no-tablespaces \''.$database.'\' | sudo gzip" | gunzip | mysql --user root --password=\'\' --execute \'SET FOREIGN_KEY_CHECKS = 0;\' \''.$name.'\'');
